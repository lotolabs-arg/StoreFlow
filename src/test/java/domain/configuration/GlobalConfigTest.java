...
class GlobalConfigTest {
    ...
    void shouldCreateConfig_WithValidMargin() {
        GlobalConfig config = new GlobalConfig(new BigDecimal("0.30"));

        assertNotNull(config.getId());
        assertEquals(new BigDecimal("0.30"), config.getDefaultProfitMargin());
    }

    @Test
    @DisplayName("Should allow high profit margins like 200% (2.0)")
    void shouldAllowHighProfitMargin() {
        GlobalConfig config = new GlobalConfig(new BigDecimal("2.0"));
        assertEquals(new BigDecimal("2.0"), config.getDefaultProfitMargin());
    }
    ...
    void shouldFail_WhenMarginIsNegative() {
        assertThrows(DomainException.class, () -> {
            new GlobalConfig(new BigDecimal("-0.10"));
        });
    }
    ...
    void shouldFail_WhenMarginIsUnrealistic() {
        assertThrows(DomainException.class, () -> {
            new GlobalConfig(new BigDecimal("50.0"));
    ...
    void shouldUpdateProfitMargin() {
        ...
        config.updateProfitMargin(new BigDecimal("0.45"));

        assertEquals(new BigDecimal("0.45"), config.getDefaultProfitMargin());
    }
}
